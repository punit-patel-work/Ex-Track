...
export default function LoginPage() {
  const { login, loginAsGuest, isMock } = useAuth()
      ...
      <div className="w-full max-w-md space-y-8 relative z-10">
        <div className="text-center">
          ...
          <h2 className="mt-6 text-3xl font-extrabold tracking-tight bg-gradient-to-r from-white via-zinc-100 to-zinc-400 bg-clip-text text-transparent">
            Welcome back to Ex-Track
          </h2>
          <p className="mt-2 text-sm text-zinc-400">
            Manage your budget, track savings, and build wealth.
          </p>
        </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => alert('Password reset emails are handled by Supabase. Check your inbox after requesting a reset from the Supabase Auth dashboard, or contact support.')}
                  className="text-xs font-semibold text-violet-400 hover:text-violet-300 transition cursor-pointer"
                >
                  Forgot password?
                </button>
              </div>
        ...
        <p className="text-center text-sm text-zinc-400">
          ...
          <Link href="/signup" className="font-semibold text-violet-400 hover:text-violet-300 hover:underline transition duration-200">
            Sign Up
          </Link>
        </p>
        ...
        {isMock && (
          <div className="text-center p-4 rounded-2xl bg-violet-950/20 border border-violet-900/30">
            ...
            <p className="text-xs text-zinc-400">
              Supabase credentials not detected in .env. Signing in as guest runs in sandbox mode, storing data locally.
            </p>
          </div>
        )}
      </div>
